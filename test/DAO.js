...
describe('DAO', () => {
    ...
    beforeEach(async () => {
        ...
        transaction = await token.connect(deployer).transfer(deployer.address, tokens(200000))
        await transaction.wait()

        transaction = await token.connect(deployer).transfer(funder.address, tokens(200000))
        await transaction.wait()

        transaction = await token.connect(deployer).transfer(admin.address, tokens(200000))
        await transaction.wait()

        transaction = await token.connect(deployer).transfer(admin2.address, tokens(200000))
        await transaction.wait()

        transaction = await token.connect(deployer).transfer(admin3.address, tokens(200000))
        await transaction.wait()

        transaction = await token.connect(deployer).transfer(recipient.address, tokens(200000))
        await transaction.wait()
    ...
    describe('\nProposal Creation', () => {
        ...
        describe('Success', () => {
            beforeEach(async () => {
                ...
                transaction = await dao.connect(funder).createProposal('Proposal 1', ether(100), recipient.address)
            ...
            it('emits a propose event', async () => {
                await expect(transaction).to.emit(dao, 'Propose')
                    .withArgs(1, ether(100), recipient.address, funder.address)
            })
        })

        describe('Failure', () => {
            it('rejects invalid amount', async () => {
                await expect(dao.connect(admin).createProposal('Proposal', ether(1000), recipient.address)).to.be.reverted
            })
